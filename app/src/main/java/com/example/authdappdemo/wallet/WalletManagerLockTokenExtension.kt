...
object WalletManagerLockTokenExtension {
    ...
    fun WalletManager.withdrawERC20FromContract(context: Activity?){
        val promptDialog =  PromptDialog(context)
        promptDialog.showLoading("")
        ...
        easyThread.execute(Runnable {
            ...
            val function = FunctionEncoder.makeFunction(
                "withdraw", arrayListOf("token", "uint256"),
                arrayListOf("token合约地址", WalletConst.mWithdrawEth) as List<Any>?, emptyList()
            )
            val encodedFunction = FunctionEncoder.encode(function)
            val rawTransaction = RawTransaction.createTransaction(
                nonce,
    ...
    fun WalletManager.depositERC20FromContract(context: Activity?){
        val promptDialog =  PromptDialog(context)
        promptDialog.showLoading("")
        ...
        easyThread.execute(Runnable {
            ...
            val function = FunctionEncoder.makeFunction(
                "transfer",
                arrayListOf("address", "uint256"),
                arrayListOf("0xd6De6eEEC1F3D775A463A5BF81897359cb2939F5", WalletConst.mWithdrawEth) as List<Any>?, emptyList()
            )
